 ...
 void EPWM_tem_Init()
{
	...
	EPwm1Regs.CMPA.half.CMPA =(unsigned int)(TEM_SP);    // set zhankongbi
	EPwm1Regs.CMPB=0x0;
	...
	EPwm1Regs.TBCTL.bit.CLKDIV = TEM_TB_CLKDIV;
	EPwm1Regs.TBCTL.bit.HSPCLKDIV = TEM_TB_HSPCLKDIV;
	EPwm1Regs.TBCTL.bit.PHSEN = 0; //  count register load form phase register   enable
	...
	EPwm1Regs.TBPRD=(unsigned int)(TEM_SP);      //the time-base counter starts from zero orX and  increments nntil the period 
 ...
 * canshu : x (0~SP)
 */
char EPWM_tem_UpdataCMPA(unsigned int x)
{
	int cycle = x;
	if(cycle < TEM_SP)
	{
	...
	{
		EALLOW;
		EPwm1Regs.CMPA.half.CMPA = (unsigned int)(TEM_SP);	
		EDIS;
		return 0;
	}
}
 ...
 */
 ...
 {
 	EALLOW;
 //	EPwm1Regs.TBCTL.bit.PHSEN = 0x0; //  TB disenable
 	EDIS;
 }
