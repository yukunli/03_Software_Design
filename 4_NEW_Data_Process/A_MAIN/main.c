...
_iq24 Cross_OutPut[BUF_SIZE1] = {0};
...
// 下载程序到Flash
    ...
    */
	...
	{	
		...
		{
			//进行点采样
			GetAD_Value(&gSampleTable,&gSampleValue);  
			Timer_flag = 0;
			SIN_On(); 
		}
		else if(SampleCount_Status_Flag == True)   //采样序列已满，进行下一步的处理
		{
			...
			IER |= M_INT8; 	//使能SCI_C中断		
		 	...
		 	//计算单路通道的信号幅值
		 	L17Value[Count] = Single_AmpValue(gDalOutPut.DAL_OutPut1,BUF_SIZE1+LOWFILT_SIZE-1 );
		 	L19Value[Count] = Single_AmpValue(gDalOutPut.DAL_OutPut2,BUF_SIZE2+LOWFILT_SIZE-1 );
		 	L22Value[Count] = Single_AmpValue(gDalOutPut.DAL_OutPut3,BUF_SIZE3+LOWFILT_SIZE-1 );
		 	Count++;
		 	if(Count == 10)
		 	{
		 		for(i = 0; i < 10; i++)
		 		...
		 		Test_gSystem_LabviewData.L17Value = sum17Value/10;
		 		Test_gSystem_LabviewData.L19Value = sum19Value/10;
		 		Test_gSystem_LabviewData.L22Value = sum22Value/10;
		 		Count = 0;
		 		sum17Value = 0;
		 		sum19Value = 0;
		 		sum22Value = 0;
...
//		 	Test_gSystem_LabviewData.L17Value = L17Value[0];
//		 	Test_gSystem_LabviewData.L19Value = L19Value[0];
//		 	Test_gSystem_LabviewData.L22Value = L22Value[0];
//		 	SendValuetoLabview(&Test_gSystem_LabviewData);
		 	//利用拟合公式计算当前瞬时水分值
