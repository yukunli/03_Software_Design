...
** 日期: 2013-12-12
** 1) 完善注释，去掉编译产生的警告
** 2) 所使用的硬件平台为STM32F103VCT6 ,使用内部RC振荡器
------------------------------------------------------------------------------------*/
...
{
//		  uint8 tem_graph[8] = {0,10,20,30,40,0,50,100};   //温度数据
...
//     uint8  value = msg->param[1];          //按钮数值
    ...
    else if(screen_id == 2 && control_id == 4)//画面2 ID为4的按键被按下了
    {
        ...
        SetButtonValue(2,3,0);                  //把启动运行的图标变为弹起来状态(画面2，控件3)
...
void OnRecvText(PCTRL_MSG msg,qsize size)
{
    ...
    if(screen_id == 1 &&  control_id == 2)
    {
        while(((uint8 *)(&msg->param))[i])
            voltage = voltage*10+ (((uint8 *)(&msg->param))[i++] - 0x30); //从接收缓冲区取出键盘输入的数字，再转换成十进制数字
        sprintf((char*)buf,"%d伏特",voltage/2);                              //获取键盘输入的数据除以2后再显示出来
        SetTextValue( 1, 5,buf);		                                  //显示到最高电压/2文本控件处(画面1，控件5)
    }
}
...
void OnRecvSlider(PCTRL_MSG msg,qsize size)
{
    ...
    control_id = PTR2U16(&msg->control_id); //控件ID
    //TODO: 用户代码
    if(screen_id ==5 && control_id == 2)   //画面5 
    {
