...
Filter11 = [-0.001638296875, 0.001976465574, 0.002134647453, 0.002753725043, 0.003676692024,
...
def DalProcess(bufsize,singledata =[]):
	...
	for k in range(0,Buf_size):
		...
		SampleBuffer1[Buf_size-k-1] = temp1/(Buf_size-k)
		SampleBuffer2[Buf_size-k-1] = temp2/(Buf_size-k)

	
	
	print "ditonglvbo SampleBuffer1:\n"
##	SampleBuffer1 = LinearConvolution(SampleBuffer1,Filter_Size,Filter,SampleBuffer1)
##	SampleBuffer2 = LinearConvolution(SampleBuffer2,Filter_Size,Filter,SampleBuffer2)
	
	for kk in range(0,Buf_size):
		temp1 = 0
		temp2 = 0
		for j in range(0,Buf_size-kk):
			temp1 += Channel_Date[j+kk]*longsinwave[j]
			temp2 += Channel_Date[j+kk]*longcoswave[j]
		SampleBuffer1[Buf_size+kk-1] = temp1/(Buf_size-kk)
		SampleBuffer2[Buf_size+kk-1] = temp2/(Buf_size-kk)
	print "SampleBuffer1 after dal\N"
	drawpicture(SampleBuffer1)
	print "SampleBuffer2 after dal\N"
	drawpicture(SampleBuffer2)
		
	for kk in range(0,2*Buf_size-1):
		
		temp1 = SampleBuffer1[2*Buf_size-kk-2]
		temp2 = SampleBuffer2[2*Buf_size-kk-2]
...
##  global DAL_OutPut
        ...
        SampleBuffer1 = [ initial_value for i in range(2*Buf_size-1)]
